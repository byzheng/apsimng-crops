...
cultivar <- ifelse(is.null(params$cultivar), "All", params$cultivar)
experiment <- ifelse(is.null(params$experiment), "All", params$experiment)
obs_sim <- params$obs_sim
cultivar_string <- ifelse(cultivar == "All", "all cultivars", sprintf("cultivar `%s`", cultivar))
experiment_string <- ifelse(experiment == "All", "all experiments", sprintf("experiment `%s`", experiment))
```
...
The table below summarizes the performance of APSIM NG for `r cultivar_string` across `r experiment_string`.


```{r tbl-performance-summary}
tbl_cap <- sprintf("Performance summary for %s across %s", cultivar_string, experiment_string)
obs_sim |> 
    dplyr::group_by(trait) |>
    myutils::summarise_model(x = "Observed", y = "Predicted", digits = 2)|>
    dplyr::ungroup() |>
    dplyr::arrange(trait) |>
...
The model performance is compared with observed and predicted values for `r cultivar_string` across `r experiment_string`. The plots below show the observed vs predicted values for different traits. Each point represents an observation from an experiment. The closer the points are to the diagonal line, the better the performance of the model for that trait.
